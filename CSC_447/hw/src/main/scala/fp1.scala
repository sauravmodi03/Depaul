...
object fp1:
  ...
  val factTest: List[Int] =
    List(fact(0),fact(2),fact(3),fact(4),fact(5))
  ...
  def sumTailAux(xs: List[Int], accumulator: Int): Int =
    xs match
      ...
      case y :: res => sumTailAux(res, y + accumulator)
  ...
  def maxList(xs: List[Int]): Int = 
    ...
    else xs.reduceLeft((x, y) => if x > y then x else y)


    // xs match{
    //     case Nil => throw new NoSuchElementException()
    //     case notNull => xs.reduceLeft((x, y) => if x > y then x else y)
    //   }
