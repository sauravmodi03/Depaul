...
object fp3:
  ...
  def allEqual(xs: List[Int]): Boolean =
    xs match{
      case Nil => true
      case xh :: Nil => true
      case xh :: xt => if (xh != xt.head) then return false
                     else allEqual(xt)
    }
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ys match{
     case Nil => Nil
     case yh::yt => if x == yh then delete1(yh,yt) 
                  else yh :: delete1(x,yt)
    }
  ...
  def removeDupes1[X](xs: List[X]): List[X] =
    xs match{
      case Nil => Nil
      case xh :: Nil => xh :: Nil
      case xh :: xt => if xh != xt.head then xh :: removeDupes1(xt)
                        else removeDupes1(xt)
    }
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    def remove [X] (x:X, xs:List[X],count:Int) :List[(Int,X)] = 
    xs match{
     case Nil => if x == Nil then List()
                else (count,x)::List()
     case yh :: yt => if x == yh then remove(x,yt,count+1) 
                   else (count,x) :: remove(yh,yt,1)
    }   
    if xs.length > 0 then remove(xs.head,xs.tail,1) 
    else List()
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    case (-1,xs) => (List(),xs)
    case (n,xh :: xt) => {
      val(l1,l2) = splitAt(n-1,xt) 
      if n > 0 then (xh :: l1,l2) 
      else (l1, xh :: l2)}
  }
  ...
  def allDistinct(xs: List[Int]): Boolean =
    xs match{
      ...
      case xh :: xt => if member(xh,xt) then return false 
      else allDistinct(xt)
  }
